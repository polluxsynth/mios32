...
static s32 do_store_pgm(const struct page *page)
{
  ...
  res = EEPROM_BlockWrite(0, current_pgm, SYNTH_GetParamsAddress(current_chan), NPARAM);
  ...
  res = EEPROM_BlockRead(0, current_pgm, buf, EEPROM_BLOCKSIZE);
...
MIOS32_MIDI_SendDebugMessage("Loading program %d\n", value);
    res = EEPROM_BlockRead(0, value, SYNTH_GetParamsAddress(current_chan), NPARAM);
    if (res == 0) SYNTH_ParamUpdate(current_chan);
  } 

#if 0
  static u8 buf[EEPROM_BLOCKSIZE];
  res = EEPROM_BlockRead(0, value, buf, EEPROM_BLOCKSIZE);
